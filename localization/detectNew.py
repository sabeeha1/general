...
def detect(save_img=False):
    ...
    # Load model Uncomment following lines if using GPU
    # model = Darknet(cfg, imgsz).cuda()
    # model.load_state_dict(torch.load(weights[0], map_location=device)['model'])
    model = Darknet(cfg, imgsz)
    model.load_state_dict(torch.load(weights, map_location=lambda storage, loc: storage)['model'])
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if save_crop:
                            pass
                            # save_one_box(xyxy, imc, file=save_dir / 'crops' / names[int(c)] / f'{Path(p).stem}.jpg', BGR=True)
                            # save_one_box(xyxy, imc, file=save_dir / 'crops' / str(i) / f'{Path(p).stem}.jpg', BGR=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='weights/groc60.pt', help='model.pt path(s)')
